...
impl MapDirection{
    fn all() -> Vec<MapDirection>{
        return vec![MapDirection::North, MapDirection::East, MapDirection::South, MapDirection::West]
    }
...
fn do_first_generation_step(map: &mut Vec<Cell>, active_coords: &mut Vec<Coords>, initialized_cells: &mut HashMap<usize, bool>){
...
fn do_next_generation_step(map: &mut Vec<Cell>, active_coords: &mut Vec<Coords>, initialized_cells: &mut HashMap<usize, bool>){
    ...
    match adjust_in_direction(&active_coord, &direction) {
        Some(next_coord) => {
            match initialized_cells.get(&get_index_from_coords(&next_coord)){
                Some(_) => create_wall(map, &active_cell_index, Some(&get_index_from_coords(&next_coord)), &direction),
                None => {
...
fn create_cell(map: &mut Map, coord: &Coords, initialized_cells: &mut HashMap<usize, bool>) -> usize{
...
fn create_passage(map: &mut Map, cell_a: &usize, cell_b: &usize, direction: &MapDirection){
    map[*cell_a].edges.insert(direction.clone(), EdgeType::Passage);
    map[*cell_b].edges.insert(direction.opposite(), EdgeType::Passage);
}
...
fn create_wall(map: &mut Map, cell_a: &usize, cell_b: Option<&usize>, direction: &MapDirection){
    ...
    match cell_b{
        Some(cell) => {
            map[*cell].edges.insert(direction.opposite(), EdgeType::Wall);
        },
        None => (),
    };
}
...
fn get_random_uninitialized_direction(active_cell: &Cell) -> MapDirection{
    let used_edges: Vec<MapDirection> = active_cell.edges.iter().map(|(key, _)| key.clone()).collect();
    let unused_directions: Vec<MapDirection> = MapDirection::all().iter().filter(|v| !used_edges.contains(v)).cloned().collect();
...
fn adjust_in_direction(active_coord: &Coords, direction: &MapDirection) -> Option<Coords>{
    match direction {
        MapDirection::North => {
            if active_coord.y == 0 {return None};
            return Some(Coords{x: active_coord.x, y: active_coord.y - 1});
        }
        MapDirection::East => {
            if active_coord.x == (WIDTH - 1) {return None};
            return Some(Coords{x: active_coord.x + 1, y: active_coord.y});
        }
        MapDirection::South => {
            if active_coord.y == (HEIGHT - 1) {return None};
            return Some(Coords{x: active_coord.x, y: active_coord.y + 1});
        }
        MapDirection::West => {
            if active_coord.x == 0 {return None};
            return Some(Coords{x: active_coord.x - 1, y: active_coord.y});
        }
    }
}
...
pub fn update_map(map: &mut Map){
    map[get_index_from_coords(&Coords{x: 15, y: 15})] = Cell::no_walls(String::from("grey"))
}
...
pub fn respond_to_player(map: &Map, map_state: &mut MapState, user_id: String, input: String) -> Option<String>{
    ...
    match player_action {
        PlayerInput::Register => {
            match map_state.player_state.get(&user_id) {
                Some(_) => (),
                None => {
                    map_state.player_state.insert(user_id, Coords{x: 10, y: 10});
                }
            }
            return Some(serde_json::to_string(&map_state).unwrap());
        },
        PlayerInput::MoveNorth => {
            let player_state = map_state.player_state.get(&user_id).unwrap().clone();
            let new_position = adjust_in_direction(&player_state, &MapDirection::North);
            match new_position {
                Some(new_position) => {
                    let next_cell = map.get(get_index_from_coords(&new_position)).clone(); 
                    match next_cell {
                        Some(next_cell) => {
                            map_state.explored_cells.insert(get_index_from_coords(&new_position), next_cell.clone());
                            map_state.player_state.insert(user_id, new_position);
                            
                            let move_command = serde_json::to_string(&MoveCommand{move_id: 2, cell: next_cell.clone()}).unwrap();
                            return Some(move_command);
                        },
                        None => return None,
                    }
        ...
        PlayerInput::MoveEast => {
            let player_state = map_state.player_state.get(&user_id).unwrap().clone();
            let new_position = adjust_in_direction(&player_state, &MapDirection::East);
            match new_position {
                Some(new_position) => {
                    let next_cell = map.get(get_index_from_coords(&new_position)).clone(); 
                    match next_cell {
        ...
        PlayerInput::MoveSouth => {
            let player_state = map_state.player_state.get(&user_id).unwrap().clone();
            let new_position = adjust_in_direction(&player_state, &MapDirection::South);
            match new_position {
                Some(new_position) => {
                    let next_cell = map.get(get_index_from_coords(&new_position)).clone(); 
                    match next_cell {
        ...
        PlayerInput::MoveWest => {
            let player_state = map_state.player_state.get(&user_id).unwrap().clone();
            let new_position = adjust_in_direction(&player_state, &MapDirection::West);
            match new_position {
                Some(new_position) => {
                    let next_cell = map.get(get_index_from_coords(&new_position)).clone(); 
                    match next_cell {
